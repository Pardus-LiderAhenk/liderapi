...
public interface UserSessionRepository extends BaseJpaRepository<UserSessionImpl, Long> {
	...
	@Query(value = "SELECT  NEW map(s.id as usr_id, s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent=a.id WHERE s.sessionEvent = 1")
	...
	@Query(value = "SELECT  NEW map(s.id as usr_id, s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent=a.id WHERE s.sessionEvent = 2")
