...
public interface UserSessionRepository extends BaseJpaRepository<UserSessionImpl, Long> {
	...
	@Query(value = "SELECT  NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent=a.id WHERE s.username LIKE %?1% AND s.sessionEvent = 1 AND s.createDate BETWEEN ?2 AND ?3")
	...
	@Query(value = "SELECT NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent = a.id WHERE a.hostname LIKE %?1%")
	Page<Map<String, Object>> findByUserSessionByDn(String dn, Pageable pageable);

	@Query(value = "SELECT  NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent=a.id WHERE s.username LIKE %?1% AND a.hostname LIKE %?2% AND s.sessionEvent = 1 AND s.createDate BETWEEN ?3 AND ?4")
	Page<Map<String, Object>> findByUserSessionUsernameAndDn(String username,String dn, Optional<Date> startDate, Optional<Date> endDate,Pageable pageable);
	
	@Query(value = "SELECT  NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN AgentImpl a ON s.agent=a.id WHERE s.sessionEvent = 2")
	Page<Map<String, Object>> findByUserSessionLogoutAll(Pageable pageable);
	
	@Query(value = "SELECT  NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s  JOIN AgentImpl a ON s.agent=a.id WHERE s.createDate BETWEEN ?1 AND ?2 ")
	...
	@Query(value = "SELECT NEW map(s.username as username, a.hostname as hostname, s.sessionEvent as sessionEvent, a.ipAddresses as ipAddresses, a.macAddresses as macAddresses, s.createDate as createDate) FROM UserSessionImpl s JOIN  AgentImpl a ON s.agent=a.id WHERE s.username LIKE %?1%")
